...
const char _program_name[] = "ffprobe";
const int _program_birth_year = 2007;
...
static inline int show_tags(WriterContext *w, AVDictionary *tags, int section_id)
{
    ...
    while ((tag = av_dict_get(tags, "", tag, AV_DICT_IGNORE_SUFFIX))) {
        [FFmpegCmd.share metaInfo:tag->key value:tag->value];
...
static int show_format(WriterContext *w, InputFile *ifile)
{
	...
	/// 获取信息
    [FFmpegCmd.share metaBasicInfo:fmt_ctx->url
                         nbstreams:fmt_ctx->nb_streams
                        nbprograms:fmt_ctx->nb_programs
                        formatname:fmt_ctx->iformat->name
                    formatlongname:fmt_ctx->iformat->long_name ? fmt_ctx->iformat->long_name : "unknown"
                           ...
                           const char *print_filename)
{
    ...
    av_dump_format(fmt_ctx, 0, filename, 0);
		...
		//	fmt_ctx->streams[fmt_ctx->nb_streams]->codec->codec_type //AVMEDIA_TYPE_AUIDO
		//	fmt_ctx->streams[fmt_ctx->nb_streams]->codec->sample_rate 160000
		//	fmt_ctx->streams[fmt_ctx->nb_streams]->codec->channels 1
		//	fmt_ctx->streams[fmt_ctx->nb_streams]->codec->sample_fmt AV_SAMPLE_FMT_S16
	// 判断音频文件是不是PCM格式
	if (fmt_ctx->nb_streams == 1 && fmt_ctx->streams[0]->codec->codec_type == AVMEDIA_TYPE_AUDIO) {
		if (fmt_ctx->streams[0]->codec->sample_rate == 16000 &&
			fmt_ctx->streams[0]->codec->channels == 1 &&
			fmt_ctx->streams[0]->codec->sample_fmt == AV_SAMPLE_FMT_S16) {
			FFmpegCmd.share.isPCM = YES;
		}
	}
...
static void ffprobe_show_program_version(WriterContext *w)
{
    ...
    print_fmt("copyright", "Copyright (c) %d-%d the FFmpeg developers",
              _program_birth_year, CONFIG_THIS_YEAR);
